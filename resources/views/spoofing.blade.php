...
<div class="container-fluid" style="background-color: white;">
 <div class="row">
  ...
  <div class="dropdown-container">
    ...
    <h1 class="Display-1">Email Spoofing</h1>
    <h4>(Sec. 463 IPC) </h4>
    <hr >
    <div class="row">
        <div class="col-md-8">
            <p>Email spoofing is the creation of email messages with a forged sender address.

The core email protocols do not have any mechanism for authentication, making it common for spam and phishing emails to use such spoofing to mislead or even prank the recipient about the origin of the message.</p>
            </div>
        <div class="col-md-4">
            <img src="https://saracenssolicitors.co.uk/wp-content/uploads/2017/08/defamation-on-social-media-slander--620x465.jpg" style=" width: 95%; height: 75%;box-shadow: 4px 6px 8px 0 rgba(0, 0, 0, 0.2), 4px 6px 20px 0 rgba(0, 0, 0, 0.19);border-radius:5%">
        </div>         
    </div>

   <div class="row">
     <div class="col-md-6">
       <h2>Punishment for Email Spoofing:</h2>
       <hr>
         <ul style="font-size:18px">
         <li>
        Imprisonment of either description for a term may extend to Threee years and shall also be liable to fine which may extend to one lakh rupees.
         </li>
             </ul>
     </div>
     <div class="col-md-6">
      ...
      <br>
      <br>
       <img src="https://blog.ipleaders.in/wp-content/uploads/2019/11/cyber-crime-in-florida.jpg" style=" width: 85%; height: 75%;box-shadow: 4px 6px 8px 0 rgba(0, 0, 0, 0.2), 4px 6px 20px 0 rgba(0, 0, 0, 0.19);border-radius:5%">
     </div>
   </div>
  ...
  <div class="row">
    <div class="col-md-12">
     <ul style="font-size:18px">
         <li><b>SPF (Sender Policy Framework):</b> This checks whether a certain IP is authorized to send mail from a given domain or an email validation protocol designed to detect and block email spoofing. This method will tell receiving mail servers whether an IP is on the list for the sending domain. Unfortunately SPF lead to false positives and the results are not satisfactory. So this still leaves the work to the receiving server.</li>
         <li><b>DKIM (Domain Key Identified Mail):</b> DKIM is pretty complicated. This method uses a private and a public key fetched by a Mail Transfer Agent (MTA). These are compared and only if it is a match the mail will be sent on. But this only signs the specified parts of the message, the message can be forwarded and the signature will still match. This is called a replay attack. The problem with DKIM is that it’s more difficult to implement.</li>
         <li><b>DMARC (Domain-based Message Authentication, Reporting, and Conformance):</b> DMARC ensures that legitimate email is properly authenticating against established DKIM and SPF standards and what actions to take and who to report to when dealing with mails that fail authentication, but unfortunately DMARC is not very widely used.</li>
         </ul>
  </div>
        </div></div></div>
</div>
